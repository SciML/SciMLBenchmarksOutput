...
const VEKT13 = 46.0
const VMHC = 1.0 / (
    FRAC6 / VEKT6 + FRAC7 / VEKT7 + FRAC8 / VEKT8 +
...
const A_emep = [
    1.23e-3, 2.0e-4, 1.45e-2, 2.2e-5, 3.0e-6,
    5.4e-5, 6.65e-5, 1.35e-5, 2.43e-5, 5.4e-4,
    2.16e-4, 5.4e-5, 3.53e-2, 8.94e-2, 3.32e-5,
    2.27e-5,
]
const B_emep = [
    0.6, 1.4, 0.4, 0.75, 1.25,
    0.79, 0.6, 0.94, 0.88, 0.79,
...
function emep!(dy, y, p, t)
    ...
    return dy[66] = rc[85] * y[15] * y[57] - (rc[149] * y[37] + rc[52]) * y[66]
end
...
sol = solve(prob, Rodas5P(), abstol = 1 / 10^10, reltol = 1 / 10^10, maxiters = Int(1.0e7), tstops = disc_times)
...
wp = WorkPrecisionSet(
    ...
    save_everystep = false, appxsol = test_sol, maxiters = Int(1.0e5), numruns = 10,
    tstops = disc_times
)
plot(wp)
...
wp = WorkPrecisionSet(
    prob, abstols, reltols, setups; verbose = SciMLLogging.None(),
    names = solnames, save_everystep = false, appxsol = test_sol, maxiters = Int(1.0e5), numruns = 10,
    tstops = disc_times
)

plot(
    wp, title = "Implicit Methods: EMEP", legend = :outertopleft, size = (1000, 500),
    xticks = 10.0 .^ (-15:1:1),
    yticks = 10.0 .^ (-6:0.3:5),
