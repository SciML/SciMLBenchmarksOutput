...
using OrdinaryDiffEqBDF, OrdinaryDiffEqExtrapolation, OrdinaryDiffEqRosenbrock, OrdinaryDiffEqSDIRK
using SciMLLogging
...
function e5!(du, u, p, t)
    ...
    return du[3] = du[2] - du[4]         # f(3) = f(2) - f(4)
end
...
ylabels = 10.0 .^ (-30:2:0)
plot(
    sol, xscale = :log10, yscale = :log10, tspan = (1.0e-5, 1.0e10),
...
wp = WorkPrecisionSet(
    probs, abstols, reltols, setups; error_estimate = :l2, verbose = SciMLLogging.None(), dense = false,
...
wp = WorkPrecisionSet(
    probs, abstols, reltols, setups; verbose = SciMLLogging.None(), dense = false,
    save_everystep = false, appxsol = test_sol, maxiters = Int(1.0e5), numruns = 10
)
plot(wp)
...
setups = [
    ...
    Dict(
        :alg => ImplicitHairerWannerExtrapolation(
