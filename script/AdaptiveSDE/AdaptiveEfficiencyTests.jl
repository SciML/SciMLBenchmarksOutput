...
for k in 1:size(probs, 1)
    ...
    for i in (1 + offset):(N + offset)
        ...
        msims[i - offset] = DiffEqBase.calculate_ensemble_errors(
            solve(
                monte_prob, SRIW1(),
                EnsembleThreads(), trajectories = 1000, abstol = 2.0^(-i - 1),
                reltol = 0, force_dtmin = true, maxiters = Int(1.0e7)
            )
        )
...
for k in 1:size(probs, 1)
    global probs, Ns, fullMeans, fullMedians, fullElapsed, fullTols
    p1[k] = Plots.plot(
        fullTols[k], fullMeans[k], xscale = :log10, yscale = :log10,
    ...
    p2[k] = Plots.plot(
        fullTols[k], fullMedians[k], xscale = :log10, yscale = :log10,
    ...
    p3[k] = Plots.plot(
        fullTols[k], fullElapsed[k], xscale = :log10, yscale = :log10,
