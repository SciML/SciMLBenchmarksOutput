...
function runAdaptive(i, k)
    sol = solve(
        prob, SRIW1(), dt = 1 / 2^(8), abstol = 2.0^(-15), reltol = 2.0^(-10),
        verbose = SciMLLogging.None(), maxiters = Int(1.0e12), qmax = qs[k]
    )
    return Int(any(isnan, sol[end]) || sol.t[end] != 1)
end
...
for k in eachindex(probs)
    ...
    for i in eachindex(qs)
        msim = solve(
            monte_prob, dt = 1 / 2^(4), SRIW1(), adaptive = true,
