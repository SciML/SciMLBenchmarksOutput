...
# Add benchmark directory to Python path so we can import our utilities
sys = pyimport("sys")
sys.path.insert(0, @__DIR__)

nn_utils = pyimport("nn_benchmark_utils")
np = pyimport("numpy")
jax = pyimport("jax")
...
function to_row_major(x::AbstractMatrix)
    # Julia is column-major (features x batch), Python expects row-major (batch x features)
    np.array(Array(x'))
end

function to_nchw(x::AbstractArray{T, 4}) where T
    # Julia WHCN -> Python NCHW
    np.array(permutedims(x, (4, 3, 1, 2)))
end
...
flux_mlp_relu = Flux.Chain(
    Flux.Dense(32, 256, relu), [Flux.Dense(256, 256, relu) for _ in 1:5]..., Flux.Dense(256, 10))

# Python models
jax_mlp_relu_params = nn_utils.create_jax_mlp_params(jax_random.PRNGKey(0))
torch_mlp_relu = nn_utils.TorchMLP("relu")

batch_sizes = [2, 8, 32, 128, 512, 2048]
...
for bs in batch_sizes
    x = randn(Float32, 32, bs)

    # Lux
    t = @benchmark $lux_mlp_relu($x, $ps_lux, $st_lux_test)
    append_timing!(results, "Lux", median(t).time / 1e9)

    # Flux
    t = @benchmark $flux_mlp_relu($x)
    ...
    t = @benchmark $sc_model($x, $ps_sc, $st_sc)
    ...
    # JAX (timed in Python)
    x_py = jnp.array(to_row_major(x))
    jax_time = pyconvert(Float64, nn_utils.time_jax_inference(nn_utils.jax_mlp_relu_forward, jax_mlp_relu_params, x_py))
    append_timing!(results, "JAX", jax_time)

    # PyTorch (timed in Python)
    x_torch = torch.from_numpy(to_row_major(x))
    torch_time = pyconvert(Float64, nn_utils.time_torch_inference(torch_mlp_relu, x_torch))
    append_timing!(results, "PyTorch", torch_time)
...
# JAX train step
jax_relu_train_step = nn_utils.make_jax_mlp_train_step(
    nn_utils.jax_mlp_relu_forward, jax_mlp_relu_params)

for bs in train_batch_sizes
    x = randn(Float32, 32, bs)
    y = randn(Float32, 10, bs)

    # Lux + Zygote
    opt_state = Optimisers.setup(Adam(1.0f-3), ps_lux)
    ...
    lux_zygote_step(lux_mlp_relu, ps_lux, st_lux, opt_state, x, y)
    t = @benchmark $lux_zygote_step($lux_mlp_relu, $ps_lux, $st_lux, $opt_state, $x, $y)
    ...
    flux_step(flux_mlp_relu, flux_opt, x, y)
    t = @benchmark $flux_step($flux_mlp_relu, $flux_opt, $x, $y)
    ...
    # JAX
    x_py = jnp.array(to_row_major(x))
    y_py = jnp.array(to_row_major(y))
    jax_time = pyconvert(Float64, nn_utils.time_jax_train_step(
        jax_relu_train_step, jax_mlp_relu_params, x_py, y_py))
    append_timing!(train_results, "JAX", jax_time)

    # PyTorch
    torch_model = nn_utils.TorchMLP("relu")
    torch_opt = torch.optim.Adam(torch_model.parameters(); lr=1e-3)
    torch_loss_fn = nn_utils.tnn.MSELoss()
    x_torch = torch.from_numpy(to_row_major(x))
    y_torch = torch.from_numpy(to_row_major(y))
    torch_time = pyconvert(Float64, nn_utils.time_torch_train_step(
        torch_model, torch_opt, torch_loss_fn, x_torch, y_torch))
    append_timing!(train_results, "PyTorch", torch_time)
...
flux_mlp_gelu = Flux.Chain(
    Flux.Dense(32, 256, gelu), [Flux.Dense(256, 256, gelu) for _ in 1:5]..., Flux.Dense(256, 10))

jax_mlp_gelu_params = nn_utils.create_jax_mlp_params(jax_random.PRNGKey(1))
torch_mlp_gelu = nn_utils.TorchMLP("gelu")


st_gelu_test = Lux.testmode(st_gelu)
...
for bs in batch_sizes
    ...
    t = @benchmark $lux_mlp_gelu($x, $ps_gelu, $st_gelu_test)
    append_timing!(gelu_results, "Lux", median(t).time / 1e9)

    t = @benchmark $flux_mlp_gelu($x)
    ...
    append_timing!(gelu_results, "JAX", jax_time)

    x_torch = torch.from_numpy(to_row_major(x))
    torch_time = pyconvert(Float64, nn_utils.time_torch_inference(torch_mlp_gelu, x_torch))
    append_timing!(gelu_results, "PyTorch", torch_time)
...
jax_gelu_train_step = nn_utils.make_jax_mlp_train_step(
    nn_utils.jax_mlp_gelu_forward, jax_mlp_gelu_params)

for bs in train_batch_sizes
    x = randn(Float32, 32, bs)
    y = randn(Float32, 10, bs)

    opt_state = Optimisers.setup(Adam(1.0f-3), ps_gelu)
    ...
    lux_gelu_step(lux_mlp_gelu, ps_gelu, st_gelu, opt_state, x, y)
    t = @benchmark $lux_gelu_step($lux_mlp_gelu, $ps_gelu, $st_gelu, $opt_state, $x, $y)
    ...
    flux_gelu_step(flux_mlp_gelu, flux_opt, x, y)
    t = @benchmark $flux_gelu_step($flux_mlp_gelu, $flux_opt, $x, $y)
    ...
    y_py = jnp.array(to_row_major(y))
    jax_time = pyconvert(Float64, nn_utils.time_jax_train_step(
        jax_gelu_train_step, jax_mlp_gelu_params, x_py, y_py))
    append_timing!(gelu_train_results, "JAX", jax_time)

    torch_model = nn_utils.TorchMLP("gelu")
    torch_opt = torch.optim.Adam(torch_model.parameters(); lr=1e-3)
    torch_loss_fn = nn_utils.tnn.MSELoss()
    x_torch = torch.from_numpy(to_row_major(x))
    y_torch = torch.from_numpy(to_row_major(y))
    torch_time = pyconvert(Float64, nn_utils.time_torch_train_step(
        torch_model, torch_opt, torch_loss_fn, x_torch, y_torch))
    append_timing!(gelu_train_results, "PyTorch", torch_time)
...
torch_mlp_bn = nn_utils.TorchMLPBN()


st_bn_test = Lux.testmode(st_bn)
...
for bs in bn_batch_sizes
    ...
    t = @benchmark $lux_mlp_bn($x, $ps_bn, $st_bn_test)
    append_timing!(bn_results, "Lux", median(t).time / 1e9)

    t = @benchmark $flux_mlp_bn($x)
    ...
    x_torch = torch.from_numpy(to_row_major(x))
    torch_time = pyconvert(Float64, nn_utils.time_torch_inference(torch_mlp_bn, x_torch))
    append_timing!(bn_results, "PyTorch", torch_time)
...
for bs in train_batch_sizes
    ...
    opt_state = Optimisers.setup(Adam(1.0f-3), ps_bn)
    ...
    lux_bn_step(lux_mlp_bn, ps_bn, st_bn, opt_state, x, y)
    t = @benchmark $lux_bn_step($lux_mlp_bn, $ps_bn, $st_bn, $opt_state, $x, $y)
    ...
    flux_bn_step(flux_mlp_bn, flux_opt, x, y)
    t = @benchmark $flux_bn_step($flux_mlp_bn, $flux_opt, $x, $y)
    ...
    # PyTorch
    torch_bn_model = nn_utils.TorchMLPBN()
    torch_opt = torch.optim.Adam(torch_bn_model.parameters(); lr=1e-3)
    torch_loss_fn = nn_utils.tnn.MSELoss()
    x_torch = torch.from_numpy(to_row_major(x))
    y_torch = torch.from_numpy(to_row_major(y))
    torch_time = pyconvert(Float64, nn_utils.time_torch_train_step(
        torch_bn_model, torch_opt, torch_loss_fn, x_torch, y_torch))
    append_timing!(bn_train_results, "PyTorch", torch_time)
...
flux_lenet = Flux.Chain(
    ...
    Flux.flatten, Flux.Dense(256, 120, relu), Flux.Dense(120, 84, relu), Flux.Dense(84, 10))

torch_lenet = nn_utils.TorchLeNet()
jax_lenet_params = nn_utils.create_jax_lenet_params(jax_random.PRNGKey(2))

lenet_batch_sizes = [2, 8, 32, 128]


st_lenet_test = Lux.testmode(st_lenet)
...
for bs in lenet_batch_sizes
    x = randn(Float32, 28, 28, 1, bs)  # WHCN

    t = @benchmark $lux_lenet($x, $ps_lenet, $st_lenet_test)
    append_timing!(lenet_results, "Lux", median(t).time / 1e9)

    t = @benchmark $flux_lenet($x)
    ...
    x_py = to_nchw(x)
    x_jnp = jnp.array(x_py)
    jax_time = pyconvert(Float64, nn_utils.time_jax_inference(nn_utils.jax_lenet_forward, jax_lenet_params, x_jnp))
    append_timing!(lenet_results, "JAX", jax_time)

    x_torch = torch.from_numpy(x_py)
    torch_time = pyconvert(Float64, nn_utils.time_torch_inference(torch_lenet, x_torch))
    append_timing!(lenet_results, "PyTorch", torch_time)
...
jax_lenet_train_step = nn_utils.make_jax_lenet_train_step(jax_lenet_params)

for bs in lenet_train_batch_sizes
    x = randn(Float32, 28, 28, 1, bs)
    y = randn(Float32, 10, bs)

    opt_state = Optimisers.setup(Adam(1.0f-3), ps_lenet)
    ...
    lux_lenet_step(lux_lenet, ps_lenet, st_lenet, opt_state, x, y)
    t = @benchmark $lux_lenet_step($lux_lenet, $ps_lenet, $st_lenet, $opt_state, $x, $y)
    ...
    flux_lenet_step(flux_lenet, flux_opt, x, y)
    t = @benchmark $flux_lenet_step($flux_lenet, $flux_opt, $x, $y)
    ...
    y_py = jnp.array(to_row_major(y))
    jax_time = pyconvert(Float64, nn_utils.time_jax_train_step(
        jax_lenet_train_step, jax_lenet_params, x_jnp, y_py))
    append_timing!(lenet_train_results, "JAX", jax_time)

    torch_lenet_model = nn_utils.TorchLeNet()
    torch_opt = torch.optim.Adam(torch_lenet_model.parameters(); lr=1e-3)
    torch_loss_fn = nn_utils.tnn.MSELoss()
    x_torch = torch.from_numpy(x_py)
    y_torch = torch.from_numpy(to_row_major(y))
    torch_time = pyconvert(Float64, nn_utils.time_torch_train_step(
        torch_lenet_model, torch_opt, torch_loss_fn, x_torch, y_torch))
    append_timing!(lenet_train_results, "PyTorch", torch_time)
...
torch_resnet = nn_utils.TorchSmallResNet()

resnet_batch_sizes = [2, 8, 32]


st_resnet_test = Lux.testmode(st_resnet)
...
for bs in resnet_batch_sizes
    x = randn(Float32, 32, 32, 3, bs)  # WHCN

    t = @benchmark $lux_resnet($x, $ps_resnet, $st_resnet_test)
    ...
    x_py = to_nchw(x)
    x_torch = torch.from_numpy(x_py)
    torch_time = pyconvert(Float64, nn_utils.time_torch_inference(torch_resnet, x_torch))
    append_timing!(resnet_results, "PyTorch", torch_time)
...
for bs in resnet_train_batch_sizes
    x = randn(Float32, 32, 32, 3, bs)
    y = randn(Float32, 10, bs)

    opt_state = Optimisers.setup(Adam(1.0f-3), ps_resnet)
    ...
    lux_resnet_step(lux_resnet, ps_resnet, st_resnet, opt_state, x, y)
    t = @benchmark $lux_resnet_step($lux_resnet, $ps_resnet, $st_resnet, $opt_state, $x, $y)
    ...
    torch_rn = nn_utils.TorchSmallResNet()
    ...
    y_torch = torch.from_numpy(to_row_major(y))
    torch_time = pyconvert(Float64, nn_utils.time_torch_train_step(
        torch_rn, torch_opt, torch_loss_fn, x_torch, y_torch))
    append_timing!(resnet_train_results, "PyTorch", torch_time)
...
println("  JAX: ", pyconvert(String, jax.__version__))
println("  PyTorch: ", pyconvert(String, torch.__version__))
