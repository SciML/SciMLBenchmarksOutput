...
function nor_gate!(F, Y, I, U1, U2, U1D, U2D)
    ...
    return F[I + 12] = -(Y[I + 12] - VBB) / RBD + IBD_func(Y[I + 12] - Y[I + 4])
end
...
function andoi_gate!(F, Y, I, U1, U2, U3, U1D, U2D, U3D)
    ...
    return F[I + 17] = -(Y[I + 17] - VBB) / RBD + IBD_func(Y[I + 17] - Y[I + 13])
end
...
function nand_gate!(F, Y, I, U1, U2, U1D, U2D)
    ...
    return F[I + 13] = -(Y[I + 13] - VBB) / RBD + IBD_func(Y[I + 13] - Y[I + 9])
end
...
function orani_gate!(F, Y, I, U1, U2, U3, U1D, U2D, U3D)
    ...
    F[I + 16] = -(Y[I + 16] - VBB) / RBS + IBS_func(Y[I + 16])
    return F[I + 17] = -(Y[I + 17] - VBB) / RBD + IBD_func(Y[I + 17] - Y[I + 9])
end
...
function FCN!(F, X, Y)
    ...
    return F[175] = -(Y[175] - VBB) / RBD + IBD_func(Y[175] - Y[171])
end
...
function dnor!(G, U, I)
    ...
    return G[I + 12] += CBDBS(U[I + 12] - U[I + 4]) * (U[I + 12] - U[I + 4])
end
...
function dandoi!(G, U, I)
    ...
    return G[I + 17] += CBDBS(U[I + 17] - U[I + 13]) * (U[I + 17] - U[I + 13])
end
...
function dnand!(G, U, I)
    ...
    return G[I + 13] += CBDBS(U[I + 13] - U[I + 9]) * (U[I + 13] - U[I + 9])
end
...
function dorani!(G, U, I)
    ...
    G[I + 15] += CGD * U[I + 15]
    G[I + 16] += CBDBS(U[I + 16]) * U[I + 16]
    return G[I + 17] += CBDBS(U[I + 17] - U[I + 9]) * (U[I + 17] - U[I + 9])
end
...
mm_test = solve(
    prob_mm, Rodas5P(), reltol = 1.0e-4, abstol = 1.0e-4,
    maxiters = 1_000_000, tstops = disc_times
)
println(
    "Rodas5P: retcode = $(mm_test.retcode), steps = $(length(mm_test.t)), ",
...
try
    dae_test = solve(
        prob_dae, IDA(), reltol = 1.0e-4, abstol = 1.0e-4,
        maxiters = 1_000_000, tstops = disc_times
    )
    println(
        "  IDA: retcode = $(dae_test.retcode), steps = $(length(dae_test.t)), ",
...
try
    daskr_test = solve(
        prob_dae, DASKR.daskr(), reltol = 1.0e-4, abstol = 1.0e-4,
        maxiters = 1_000_000, tstops = disc_times
    )
    println(
        "  DASKR: retcode = $(daskr_test.retcode), steps = $(length(daskr_test.t)), ",
...
try
    dassl_test = solve(
        prob_dae, DASSL.dassl(), reltol = 1.0e-4, abstol = 1.0e-4,
        maxiters = 1_000_000, tstops = disc_times
    )
    println(
        "  DASSL: retcode = $(dassl_test.retcode), steps = $(length(dassl_test.t)), ",
...
try
    ...
    mtk_test = solve(
        prob_mtk, Rodas5P(), reltol = 1.0e-4, abstol = 1.0e-4,
        maxiters = 1_000_000, tstops = disc_times
    )
...
if mtk_success
    global mtk_ref = solve(
        prob_mtk, Rodas5P(), reltol = 1.0e-8, abstol = 1.0e-8,
        maxiters = 10_000_000, tstops = disc_times
    )
    println(
...
plot(
    ref_sol, idxs = [224], title = "S₀ Output Signal (V₄₉)",
    xlabel = "Time", ylabel = "Voltage", lw = 1.5, legend = false
)


plot(
    ref_sol, idxs = [305], title = "S₁ Output Signal (V₁₃₀)",
    xlabel = "Time", ylabel = "Voltage", lw = 1.5, legend = false
)


plot(
    ref_sol, idxs = [323], title = "Carry Output Signal C (V₁₄₈)",
    xlabel = "Time", ylabel = "Voltage", lw = 1.5, legend = false
)


plot(
    ref_sol, idxs = [224, 305, 323],
...
println("-"^80)
for (name, idx, ref_val) in [
        ("y(224) S₀", 224, archimede_y224),
    ...
    status = relerr < 1.0e-6 ? "✓" : (relerr < 1.0e-3 ? "~" : "✗")
...
wp = WorkPrecisionSet(
    ...
    maxiters = Int(1.0e6), numruns = 1, tstops = disc_times
)
...
wp = WorkPrecisionSet(
    ...
    saveat = ts, maxiters = Int(1.0e6), numruns = 1,
