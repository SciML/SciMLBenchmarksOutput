...
const am_m4 = 0.00706;  const am_m5 = 0.0705;  const am_m6 = 0.00706
const am_m7 = 0.05498
...
# Moments of inertia
const am_i1 = 2.194e-6;  const am_i2 = 4.41e-7;  const am_i3 = 5.255e-6
...
const am_d = 28.0e-3;     const am_da = 0.0115
const am_e = 0.02;      const am_ea = 0.01421
const am_rr = 7.0e-3;      const am_ra = 9.2e-4;    const am_l0 = 0.07785
const am_ss = 35.0e-3;     const am_sa = 0.01874;   const am_sb = 0.01043
const am_sc = 18.0e-3;     const am_sd = 0.02
...
y0[5] = 0.48736497954384255022559895353
...
y0[22] = 98.566870396241089605765498217
...
function andrews_rhs_full!(du, u, p, t)
    ...
    for i in 1:7
        du[14 + i] = -f[i]
        for j in 1:7
            du[14 + i] += M[i, j] * u[14 + j]
        end
        for j in 1:6
            du[14 + i] += G[j, i] * u[21 + j]
        end
    end
    # Rows 22-27: 0 = g(q)
    for i in 1:6
...
ref_sol = solve(
    prob_mm, radau5_alg; abstol = 3.0e-8, reltol = 3.0e-8, dt = 1.0e-6,
    maxiters = Int(1.0e6)
)
...
plot(
    ref_sol; idxs = [8, 9, 10, 11, 12, 13, 14],
    label = ["v₁" "v₂" "v₃" "v₄" "v₅" "v₆" "v₇"],
...
plot(
    ref_sol; idxs = [22, 23, 24, 25, 26, 27],
    label = ["λ₁" "λ₂" "λ₃" "λ₄" "λ₅" "λ₆"],
...
plot(
    ref_sol.t, max_g, yscale = :log10,
    title = "Constraint Violation Along Trajectory",
    xlabel = "t", ylabel = "max|g(q)|",
    lw = 2, legend = false, ylims = (1.0e-16, 1.0e-6)
)


println("=== Mass-Matrix ODE Form ===")
for (name, alg) in [
        ("Rodas5P(autodiff=AutoFiniteDiff())", Rodas5P(autodiff = AutoFiniteDiff())),
    ...
    print("  $name: ")
    try
        sol = solve(prob_mm, alg, reltol = 1.0e-6, abstol = 1.0e-6, maxiters = 100000)
...
print("  IDA: ")
try
    sol_dae = solve(prob_dae, IDA(), reltol = 1.0e-6, abstol = 1.0e-6)
...
try
    sol_dae2 = solve(prob_dae, IDA(init_all = false), reltol = 1.0e-6, abstol = 1.0e-6)
...
for (name, alg) in [
    ...
    print("  $name: ")
    try
        sol_mtk = solve(prob_mtk, alg, reltol = 1.0e-6, abstol = 1.0e-6, maxiters = 100000)
...
abstols = [3.0e-5, 1.0e-5, 3.0e-6, 3.0e-7, 1.0e-7]
reltols = [3.0e-5, 1.0e-5, 3.0e-6, 3.0e-7, 1.0e-7]

setups = [Dict(:prob_choice => 1, :alg => radau5_alg)]

wp = WorkPrecisionSet(
    probs, abstols, reltols, setups;
    save_everystep = false, appxsol = refs, maxiters = Int(1.0e5), numruns = 3,
    dt = 1.0e-6
)
