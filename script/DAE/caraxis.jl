...
daeprob = DAEProblem(
    caraxis_residual!, du0_dae, u0_mm, tspan;
...
ref_sol = solve(rscprob, radau5_alg; abstol = 1.0e-12, reltol = 1.0e-12)
...
for (name, alg) in [
        ("Rodas4", Rodas4()), ("Rodas5P", Rodas5P()),
        ("RadauIIA5", RadauIIA5()), ("FBDF", FBDF()), ("QNDF", QNDF()), ("NordsieckBDF", NordsieckBDF()),
    ]
    sol = solve(mmprob, alg; reltol = 1.0e-5, abstol = 1.0e-5, maxiters = Int(1.0e3))
...
for (name, alg) in [
        ("Rodas5P", Rodas5P()), ("RadauIIA5", RadauIIA5()),
        ("FBDF", FBDF()), ("QNDF", QNDF()), ("NordsieckBDF", NordsieckBDF()),
    ]
    sol = solve(mtkprob, alg; reltol = 1.0e-8, abstol = 1.0e-8, maxiters = Int(1.0e3))
    println("  ", rpad(name, 12), " → ", sol.retcode)
end
...
for (name, alg) in [("IDA", IDA()), ("DASSL", DASSL.dassl()), ("DASKR", DASKR.daskr())]
    try
        sol = solve(daeprob, alg; reltol = 1.0e-5, abstol = 1.0e-5, maxiters = Int(1.0e3))
        println("  ", rpad(name, 12), " → ", sol.retcode)
    catch e
...
plot(
    ref_sol.t, [g1_plot g2_plot]; yscale = :log10,
