...
function water_rhs!(f, y, p, t)
    ...
    @inbounds for (idx, node) in enumerate(NETFLO_NODES)
        ...
        for k in NODE_INFLOWS[node]
            netflo += y[k]
        end
        for k in NODE_OUTFLOWS[node]
            netflo -= y[k]
        end
        f[36 + idx] = netflo
    end
...
for node in 1:NNODES
    ...
    if node == 10
        netflo_expr -= eout10_s
    end
    for k in 1:NPIPES
        if PIPES[k][2] == node
            netflo_expr += phi_vars[k]
        end
        if PIPES[k][1] == node
            netflo_expr -= phi_vars[k]
        end
...
println(
    "MTK index-reduced: $(length(ModelingToolkit.unknowns(water_sys))) states ",
    "(from 49 original)"
)
...
# Mass-matrix form
for (name, alg) in [
        ("Rodas5P", Rodas5P()), ("Rodas4P", Rodas4P()),
        ("FBDF", FBDF()), ("QNDF", QNDF()), ("NordsieckBDF", NordsieckBDF()),
        ("rodas (ODEInterface)", rodas()),
        ("RadauIIA5", RadauIIA5()),
    ]
    try
        sol = solve(prob_mm, alg, reltol = 1.0e-6, abstol = 1.0e-6, maxiters = 1_000_000)
...
for (name, alg) in [("IDA", IDA()), ("DFBDF", DFBDF()), ("DNordsieckBDF", DNordsieckBDF())]
    try
        sol = solve(prob_dae, alg, reltol = 1.0e-6, abstol = 1.0e-6, maxiters = 1_000_000)
...
for (name, alg) in [("Rodas5P", Rodas5P()), ("FBDF", FBDF()), ("NordsieckBDF", NordsieckBDF())]
    try
        sol = solve(prob_mtk, alg, reltol = 1.0e-6, abstol = 1.0e-6, maxiters = 1_000_000)
...
ref_sol = solve(
    prob_mm, Rodas5P(), reltol = 1.0e-9, abstol = 1.0e-9,
    maxiters = 10_000_000
)
...
ref_vals = [
    0.229848829647743e-2, 0.1188984650746585e-2, 0.1109503645730845e-2,
    ...
    0.2397639310739395e-2, 0.2397639310739395e-2, 0.334858143045418e-2,
    ...
    0.4751940452918529e-1, 0.4751940452918529e-1, 0.424921743360116e-1,
...
plot(
    ref_sol, idxs = 1:6, title = "Pipe Flows φ₁–φ₆",
...
plot(
    ref_sol, idxs = 7:12, title = "Pipe Flows φ₇–φ₁₂",
    xlabel = "Time [s]", ylabel = "Flow [m³/s]", lw = 1.5,
    layout = (2, 3), size = (900, 500)
)
...
plot(
    ref_sol, idxs = 13:18, title = "Pipe Flows φ₁₃–φ₁₈",
    xlabel = "Time [s]", ylabel = "Flow [m³/s]", lw = 1.5,
    layout = (2, 3), size = (900, 500)
)
...
plot(
    ref_sol, idxs = 19:24, title = "Friction Factors λ₁–λ₆",
...
plot(
    ref_sol, idxs = 25:30, title = "Friction Factors λ₇–λ₁₂",
    xlabel = "Time [s]", ylabel = "λ", lw = 1.5,
    layout = (2, 3), size = (900, 500)
)
...
plot(
    ref_sol, idxs = 31:36, title = "Friction Factors λ₁₃–λ₁₈",
    xlabel = "Time [s]", ylabel = "λ", lw = 1.5,
    layout = (2, 3), size = (900, 500)
)
...
plot(
    ref_sol, idxs = 37:49, title = "Node Pressures p₁–p₁₃",
