...
data_sol = solve(prob, Tsit5(), saveat = time_points, reltol = 1.0e-9, abstol = 1.0e-9)
...
obj = build_loss_objective(
    prob, Vern9(), L2Loss(t_concrete, data),
    tstops = t_concrete, reltol = 1.0e-9, abstol = 1.0e-9
)
...
obj = build_loss_objective(
    prob, Vern9(), L2Loss(t, data), Optimization.AutoForwardDiff(),
    tstops = t, reltol = 1.0e-9, abstol = 1.0e-9
)
optprob = OptimizationProblem(obj_short, loc_init, lb = first.(loc_bounds), ub = last.(loc_bounds))
