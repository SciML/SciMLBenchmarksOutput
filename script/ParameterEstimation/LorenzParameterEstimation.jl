...
LooserBounds = Tuple{Float64, Float64}[
    (0, 22), (0, 60), (
        0, 6,
    ),
...
# Generate Data
data_sol_short = solve(prob_short, Vern9(), saveat = t_short, reltol = 1.0e-9, abstol = 1.0e-9)
data_short = convert(Array, data_sol_short) # This operation produces column major dataset obs as columns, equations as rows
data_sol = solve(prob, Vern9(), saveat = time_points, reltol = 1.0e-9, abstol = 1.0e-9)
data = convert(Array, data_sol)


plot(data_sol_short, vars = (1, 2, 3)) # the short solution
plot(data_sol, vars = (1, 2, 3)) # the longer solution
interpolation_sol = solve(prob, Vern7(), saveat = time_points, reltol = 1.0e-12, abstol = 1.0e-12)
...
plot(
    plot(xyzt, xyz), plot(xy, xz, yz, layout = (1, 3), w = 1), layout = (2, 1), size = (
        800, 600,
    )
)
...
@btime res1 = solve(optprob, BBO_adaptive_de_rand_1_bin(), maxiters = 7.0e3)
# Tolernace is still too high to get close enough


obj_short = build_loss_objective(
    prob_short, Tsit5(), L2Loss(t_short, data_short), tstops = t_short, reltol = 1.0e-9
)
optprob = OptimizationProblem(obj_short, LocIniPar, lb = xlow_bounds, ub = xhigh_bounds)
@btime res1 = solve(optprob, BBO_adaptive_de_rand_1_bin(), maxiters = 7.0e3)
...
obj_short = build_loss_objective(
    prob_short, Vern9(), L2Loss(t_short, data_short),
    tstops = t_short, reltol = 1.0e-9, abstol = 1.0e-9
)
optprob = OptimizationProblem(obj_short, LocIniPar, lb = xlow_bounds, ub = xhigh_bounds)
@btime res1 = solve(optprob, BBO_adaptive_de_rand_1_bin(), maxiters = 7.0e3)
...
obj_short = build_loss_objective(
    prob_short, Vern9(), L2Loss(t_short, data_short),
    Optimization.AutoForwardDiff(), tstops = t_short, reltol = 1.0e-9, abstol = 1.0e-9
)


opt = Opt(:GN_ORIG_DIRECT_L, 3)
optprob = OptimizationProblem(obj_short, GloIniPar, lb = first.(LooserBounds), ub = last.(LooserBounds))
@btime res1 = solve(optprob, opt, maxiters = 10000, xtol_rel = 1.0e-12)
...
opt = Opt(:GN_ISRES, 3)
@btime res1 = solve(optprob, opt, maxiters = 10000, xtol_rel = 1.0e-12) # Accurate to single precision 8.2 seconds


opt = Opt(:GN_ESCH, 3)
@btime res1 = solve(optprob, opt, maxiters = 10000, xtol_rel = 1.0e-12) # Approximately accurate, good starting values for local optimization
...
obj = build_loss_objective(
    prob, Vern9(), L2Loss(time_points, data),
    tstops = time_points, reltol = 1.0e-9, abstol = 1.0e-9
)
optprob = OptimizationProblem(obj, GloIniPar, lb = first.(LooserBounds), ub = last.(LooserBounds))
@btime res1 = solve(optprob, BBO_adaptive_de_rand_1_bin(); maxiters = 4.0e3) # Default adaptive_de_rand_1_bin_radiuslimited 33 sec [10.2183, 24.6711, 2.28969]
...
opt = Opt(:GN_CRS2_LM, 3)
@btime res1 = solve(optprob, opt, maxiters = 20000, xtol_rel = 1.0e-12) # Hit and miss. converge approximately accurate values for local opt.91 seconds


opt = Opt(:GN_ISRES, 3)
@btime res1 = solve(optprob, opt, maxiters = 50000, xtol_rel = 1.0e-12) # Approximately accurate within local bounds


opt = Opt(:GN_ESCH, 3)
@btime res1 = solve(optprob, opt, maxiters = 20000, xtol_rel = 1.0e-12) # Approximately accurate


opt = Opt(:LN_BOBYQA, 3)
optprob = OptimizationProblem(obj_short, LocIniPar, lb = xlow_bounds, ub = xhigh_bounds)
@btime res1 = solve(optprob, opt, maxiters = 10000, xtol_rel = 1.0e-12) # Claims SUCCESS but does not iterate to the true values.


opt = Opt(:LN_NELDERMEAD, 3)
@btime res1 = solve(optprob, opt, maxiters = 10000, xtol_rel = 1.0e-12) # Inaccurate final values
