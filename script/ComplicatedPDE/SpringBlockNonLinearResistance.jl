...
const m = 2.5;
#driving speed
const v = 34 * 1.0e-3 / (365 * 24 * 60 * 60);
#stiffness
const kp = 8.0;
const kc = 44.0;
...
const τ0 = 0.4;
const Dc = get_parameterDistribution(0.004, 1.0e2 * 0.004);
const a = 0.015;
...
function f0(du, u, p, t)
    ...
    return @inbounds begin
        ...
        du[Ny, Nx, 2] = @. 1 / m * kc * (u[Ny, Nx - 1, 1] + u[Ny - 1, Nx, 1] - 2 * u[Ny, Nx, 1]) -
            1 / m * kp * u[Ny, Nx, 1] -
            1 / m * σn[Ny, Nx] * a * asinh(
            u[
                Ny, Nx, 2,
            ] / 2 / v0 * exp((τ0 + b[Ny, Nx] * log(v0 * abs(u[Ny, Nx, 3]) / Dc[Ny, Nx])) / a)
        )
    end
end
...
function get_IC()
    ...
    u0 = solve(probN, NewtonRaphson(), reltol = 1.0e-8, abstol = 1.0e-12).u
    ...
    rn = randn(rng, Int(1.0e7))
    s = GaussianRandomFields.sample(grf, xi = rn[1:randdim(grf)])
    u0[:, :, 1] = (1.0 .+ 0.001 .- 1.0e-7 .* s[1:Ny, 1:Nx]) .* u0[:, :, 1] #makes sure only forward acceleration takes place when launching the simulation
    return u0
end
...
global sol, tcpu,
    ...
    prob_mtk1, solver, reltol = reltol, abstol = abstol, maxiters = Int(1.0e12),
    save_everystep = false, dtmin = 1.0e-20, callback = cb1
); #about 55 sec
...
global sol, tcpu,
    ...
    gctime,
    memallocs = @timed solve(
    prob_mtk2, solver, reltol = reltol, abstol = abstol, maxiters = Int(1.0e12),
    save_everystep = false, dtmin = 1.0e-20, callback = cb2
); #about 175 sec
