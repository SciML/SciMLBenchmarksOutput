...
function benchmark(prob)
    sol = solve(prob, MIRK6(), dt = 0.01, abstol = 1.0e-6)
    testsol = TestSolution(sol)
    wps = WorkPrecisionSet(prob, abstols, reltols, getfield.(solvers_all, :solver); names = getfield.(solvers_all, :name), appxsol = testsol, maxiters = Int(1.0e4))
...
function plot_wpd(wp_set)
    return fig = begin
        ...
        with_theme(plot_theme) do
            ...
            Legend(
                fig[1, 2], [[l, sc] for (l, sc) in zip(ls, scs)],
    ...
    with_theme(plot_theme) do
        ...
        for i in 1:3, j in 1:5
            ...
            for wpᵢ in wp.wps
                ...
                l = lines!(
                    ax, errs, times; color = colors[idx], linewidth = 5,
        ...
        fig[0, :] = Label(
            fig, "Work-Precision Diagram for 15 Nonlinear Test Problems",
            fontsize = 24, tellwidth = false, font = :bold
        )
        ...
        Legend(
            fig[:, 6], [[l, sc] for (l, sc) in zip(ls, scs)],
            labels, "BVP Solvers";
