...
@time sol = solve(
    prob, AutoVern7(Rodas5P(autodiff = AutoFiniteDiff()));
    dt = 1.0e-4, abstol = 1.0e-14, reltol = 1.0e-14
);
...
@time wp = WorkPrecisionSet(
    ...
    save_everystep = false, appxsol = test_sol, maxiters = Int(1.0e7)
);
...
multipliers = 0.5 .^ (0:3)
setups = [
    Dict(:alg => IMEXEuler(), :dts => 1.0e-3 * multipliers),
...
multipliers = 0.5 .^ (0:3)
setups = [
    Dict(:alg => NorsettEuler(krylov = true, m = 5), :dts => 1.0e-3 * multipliers),
    Dict(:alg => NorsettEuler(krylov = true, m = 20), :dts => 1.0e-3 * multipliers),
...
multipliers = 0.5 .^ (0:4)
setups = [
    Dict(:alg => ETDRK3(krylov = true, m = 20), :dts => 1.0e-3 * multipliers),
    Dict(:alg => ETDRK4(krylov = true, m = 20), :dts => 1.0e-3 * multipliers),
    Dict(:alg => HochOst4(krylov = true, m = 20), :dts => 1.0e-3 * multipliers),
]
labels = hcat(
    "ETDRK3 (m=20)",
    "ETDRK4 (m=20)",
