...
```
Main.var"##WeaveSandBox#277".PreSynapseParams
```
...
```
Main.var"##WeaveSandBox#277".SynapseParams
```
...
Rodrigues et al. [1] synapse model that replaces NMDA model with fully state-based one from Jahr and Stevens, plus three types of VGCCs (R-type, T-type and L-type), from Magee and Johnston [2].
...
Alternatively, for the JumpProcesses package we need to define the jumps. First we define a macro to aid us in the definition of the jumps.
...
```
Main.var"##WeaveSandBox#277".@j_jump
```
...
`R_synapse` and `J_synapse` define the exact same discrete problem. The comments at the end of the line in `J_synapse` correspond to the rate index defined in `R_synapse`. For example, in `R_synapse` we have `rate[1]`
...
Also, note that we modify the order of jumps in `J_synapse` compared to `R_synapse` because we bundle `ConstantRateJump` and `VariableRateJump` separately. Algorithms in `JumpProcesses` will approach each of these jumps in a different way.
...
The `JumpProcesses` algorithms allow us to save at regular intervals with `saveat`. Thus, rather than saving when a jump occurs, we save at the same average frequency as obtained with `PDMP`.
...
SciMLBenchmarks.weave_file("benchmarks/HybridJumps","Synapse.jmd")
```
