...
```
81.994371 seconds (50.86 M allocations: 4.381 GiB, 1.60% gc time, 5.83% co
...
i = 9
i = 10
 45.153942 seconds (31.00 M allocations: 2.932 GiB, 7.64% gc time, 13.35% c
ompilation time)
```
...
i = 9
i = 10
 41.084886 seconds (26.40 M allocations: 2.782 GiB, 8.09% gc time, 6.62% co
mpilation time)
```
...
```
39.671797 seconds (7.81 M allocations: 561.368 MiB, 1.55% gc time, 5.58% c
ompilation time)
```
...
```
121.695624 seconds (6.62 M allocations: 510.775 MiB, 0.28% gc time, 1.45% c
ompilation time)
```
...
```
119.858762 seconds (4.62 M allocations: 461.264 MiB, 0.27% gc time)
```
...
```
24.580287 seconds (2.15 M allocations: 215.586 MiB, 0.43% gc time)
```
...
```
119.234981 seconds (2.63 M allocations: 263.795 MiB, 0.09% gc time)
```
...
```
16.649722 seconds (1.98 M allocations: 198.585 MiB, 0.69% gc time)
```
...
```
120.091188 seconds (5.40 M allocations: 538.682 MiB, 0.21% gc time)
```
...
```
15.593561 seconds (1.45 M allocations: 145.713 MiB, 0.24% gc time)
```
...
```
11.667760 seconds (1.74 M allocations: 174.522 MiB, 0.68% gc time, 0.31% c
ompilation time)
```
...
i = 9
i = 10
 40.657158 seconds (19.05 M allocations: 2.335 GiB, 11.03% gc time)
```
...
i = 9
i = 10
 38.561849 seconds (20.00 M allocations: 2.434 GiB, 8.68% gc time)
```
...
SciMLBenchmarks.weave_file("benchmarks/StiffSDE","Oval2LongTimes.jmd")
```
