...
sol = solve(prob,SOSRI(),dt=(1/2)^(18),progress=true,
          saveat=0.1,abstol=1e-5,reltol=1e-3,maxiters=1e7,force_dtmin=true,
          controller=PIController(SOSRI(); qmax=1.125));
Random.seed!(250)
prob = remake(prob,tspan = (0.0,500.0))
@time sol = solve(prob,SOSRI(),dt=(1/2)^(18),progress=true,
    saveat=0.1,abstol=1e-5,reltol=1e-3,maxiters=1e7,force_dtmin=true,
    controller=PIController(SOSRI(); qmax=1.125));
```

```
12.131854 seconds (126.70 k allocations: 12.854 MiB, 1.07% gc time)
```



```julia
println(maximum(@view sol[2, :]))
using Plots; gr()
lw = 2
lw2 = 3
p1 = plot(sol,idxs=(0,16),
...
sol = solve(prob,SRIW1(),dt=(1/2)^(18),
          progress=false,force_dtmin=true,
          saveat=0.1,abstol=1e-6,reltol=1e-4,
          controller=PIController(SRIW1(); qmax=1.125))
...
```
EM
  0.491690 seconds (124 allocations: 14.844 KiB)
SRI
  0.558882 seconds (347 allocations: 38.781 KiB)
SRIW1
  0.221304 seconds (181 allocations: 21.609 KiB)
SRI Adaptive
  0.591538 seconds (601 allocations: 64.062 KiB)
length(sol.t) = 11
SRIW1 Adaptive
  0.019293 seconds (431 allocations: 46.734 KiB)
length(sol.t) = 11
11
```
...
SciMLBenchmarks.weave_file("benchmarks/StiffSDE","Oval2LongRun.jmd")
```
