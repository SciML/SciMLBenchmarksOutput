...
## IDA Reference Solve (DAE form)

IDA on the DAE formulation goes unstable near the end of integration
(t ≈ 29.4 out of [0, 30]).  With the Sundials 7 C library (Sundials.jl v5)
the error-test failure is not recoverable at high precision, so we use the
ODE reference from CVODE_BDF above instead.
...
```
IDA retcode: Success, t_final: 30.0 (tspan ends at 30.0)
```
