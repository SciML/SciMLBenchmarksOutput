...
using OrdinaryDiffEq, ODEInterfaceDiffEq, LSODA, Sundials, DiffEqDevTools, StaticArrays,
      IRKGaussLegendre
using OrdinaryDiffEqAdamsBashforthMoulton, OrdinaryDiffEqExtrapolation, OrdinaryDiffEqFIRK, OrdinaryDiffEqHighOrderRK, OrdinaryDiffEqLowOrderRK, OrdinaryDiffEqVerner
using SciMLLogging
using Plots;
gr()
...
function f(u, p, t)
    @inbounds begin
        ...
        du1 = u[3]
        du2 = u[4]
        du3 = u[1] + 2u[4] - threebody_μ′*(u[1]+threebody_μ)/D₁ -
              threebody_μ*(u[1]-threebody_μ′)/D₂
...
prob = ODEProblem{true, SciMLBase.FullSpecialize}(
    f, [0.994, 0.0, 0.0, parse(Float64, "-2.00158510637908252240537862224")], tspan)
probstatic = ODEProblem{false}(
    f, SA[0.994, 0.0, 0.0, parse(Float64, "-2.00158510637908252240537862224")], tspan)
...
@show sol.u[1] - sol.u[end]
@show sol.u[end] - prob.u0;
```

```
sol.u[1] - sol.u[end] = [1.842501706761368e-10, 5.474913321311391e-10, 8.93
2818378239858e-8, 2.8677876517946288e-8]
sol.u[end] - prob.u0 = [-1.842501706761368e-10, -5.474913321311391e-10, -8.
932818378239858e-8, -2.8677876517946288e-8]
```
...
setups = [Dict(:alg=>DP5())
          ...
          Dict(:alg=>Tsit5(), :prob_choice => 2)
          Dict(:alg=>dopri5())];
wp = WorkPrecisionSet(probs, abstols, reltols, setups; appxsol = test_sol, verbose = SciMLLogging.None(), numruns = 100)
plot(wp)
```
...
```julia
wp = WorkPrecisionSet(probs, abstols, reltols, setups; appxsol = test_sol,
    dense = false, numruns = 100, verbose = SciMLLogging.None())
plot(wp)
```
...
⌅ [58bc7355] IRKGaussLegendre v1.0.4
...
⌃ [bbf590c4] OrdinaryDiffEqCore v4.16.0
  ...
  [dc97f408] OrdinaryDiffEqSIMDRK v2.2.0
