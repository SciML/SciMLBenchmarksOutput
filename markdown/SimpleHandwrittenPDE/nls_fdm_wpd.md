...
title: "Nonlinear Schrödinger Equation FDM Work-Precision Diagrams"
---
...
```math
\begin{align}
    i\partial_t \psi(t,x) + \alpha\partial_x^2 \psi(t,x) + 2|\psi(t,x)|^2 \psi(t,x) &= 0, \\
    \psi(0,x) &= \cos\left(\frac{\pi x}{L}\right), \\
    \psi(t,-L) &= \psi(t,L).
\end{align}
```

Splitting into real and imaginary parts $\psi = u + iv$, the system becomes:
```math
\begin{align}
    \partial_t u &= -\alpha\partial_x^2 v - 2(u^2 + v^2)v, \\
    \partial_t v &= \alpha\partial_x^2 u + 2(u^2 + v^2)u.
\end{align}
```

The linear part is the block operator $\alpha\begin{bmatrix} 0 & -D_x^2 \\ D_x^2 & 0 \end{bmatrix}$ which has purely imaginary eigenvalues (dispersive, not dissipative). This gives the NLS fundamentally different stiffness characteristics from diffusive PDEs like Burgers or Allen-Cahn: the spectrum lies on the imaginary axis rather than the negative real axis. The parameter $\alpha = 5$ controls the dispersive stiffness.
...
We define an `AutoSparse` backend for stiff solvers to automatically detect the Jacobian sparsity pattern using `SparseConnectivityTracer` and exploit it via graph coloring. The finite-difference stencil produces a genuinely sparse Jacobian (only 1.2% nonzero at $N = 256$), so this gives a large speedup over dense differentiation.

```julia
autosparse_ad = AutoSparse(
    AutoForwardDiff();
...
```
264.596900 seconds (10.38 M allocations: 775.464 MiB, 0.14% gc time, 4.73% 
compilation time)
```


![](figures/nls_fdm_wpd_4_1.png)



## Work-Precision Diagrams

### Stiff Methods with Automatic Sparsity

The Jacobian of the NLS system has sparse structure from the finite-difference stencil. Using `AutoSparse` with `SparseConnectivityTracer`, stiff solvers automatically detect the sparsity pattern and use sparse coloring for efficient differentiation and factorization. Without this, they compute a dense $(2N) \times (2N)$ Jacobian.

```julia
abstols = 0.1 .^ (5:8)
reltols = 0.1 .^ (2:5)
setups = [
    Dict(:alg => Rodas5(autodiff=autosparse_ad)),
    Dict(:alg => Rodas5P(autodiff=autosparse_ad)),
    Dict(:alg => Rodas5(autodiff=AutoFiniteDiff())),
    Dict(:alg => Tsit5()),
    Dict(:alg => Vern7()),
]
labels = hcat(
...
plot(wp, label=labels, markershape=:auto, title="Stiff (AutoSparse) vs Explicit")
```
...
Tsit5
Vern7
184.570791 seconds (48.76 M allocations: 3.462 GiB, 1.09% gc time, 35.90% c
ompilation time)
```


![](figures/nls_fdm_wpd_5_1.png)
...
plot(wp, label=labels, markershape=:auto, title="Stiff (AutoSparse) vs Explicit, Low Tolerances")
```
...
Vern7
Vern9
 10.212644 seconds (10.25 M allocations: 872.449 MiB, 3.79% gc time, 70.96%
 compilation time)
```


![](figures/nls_fdm_wpd_6_1.png)



### Non-Stiff Explicit Methods

The NLS has purely imaginary (dispersive) eigenvalues, unlike diffusive PDEs which have negative real eigenvalues. This means explicit methods can take large adaptive steps without instability, since the solution oscillates rather than blowing up. For smooth initial conditions, the adaptive error control finds that few steps are needed.
...
Vern9
DP8
 13.678174 seconds (3.52 M allocations: 312.872 MiB, 0.68% gc time, 75.04% 
compilation time)
```


![](figures/nls_fdm_wpd_7_1.png)
...
CNLF2
SBDF2
 50.139721 seconds (14.16 M allocations: 1.078 GiB, 3.04% gc time, 40.12% c
ompilation time)
```


![](figures/nls_fdm_wpd_8_1.png)
...
ETDRK2 (m=5)
ETDRK2 (m=20)
 98.408973 seconds (26.48 M allocations: 4.744 GiB, 2.48% gc time, 20.08% c
ompilation time)
```


![](figures/nls_fdm_wpd_9_1.png)
...
CNAB2 (Krylov)
ETDRK2 (Krylov, m=20)
 50.387307 seconds (7.11 M allocations: 2.105 GiB, 1.09% gc time, 10.84% co
mpilation time)
```


![](figures/nls_fdm_wpd_10_1.png)
...
Note: `KenCarp3`/`KenCarp4`/`KenCarp5` with dense linear solvers were tested but stall at several seconds per solve due to the $(2N) \times (2N)$ dense linear system. They are excluded from these diagrams. ARKODE with GMRES performs well as the adaptive IMEX method.
...
ARKODE4 (GMRES)
ARKODE5 (GMRES)
 32.072861 seconds (11.05 M allocations: 503.815 MiB, 2.27% gc time, 11.34%
 compilation time)
```


![](figures/nls_fdm_wpd_11_1.png)
...
ETDRK4 (m=20)
HochOst4 (m=20)
357.643450 seconds (50.59 M allocations: 14.129 GiB, 1.89% gc time, 5.08% c
ompilation time)
```


![](figures/nls_fdm_wpd_12_1.png)
...
ETDRK3 (m=20)
ETDRK4 (m=20)
203.068524 seconds (25.23 M allocations: 7.875 GiB, 0.59% gc time)
```


![](figures/nls_fdm_wpd_13_1.png)
...
SciMLBenchmarks.weave_file("benchmarks/SimpleHandwrittenPDE","nls_fdm_wpd.jmd")
```
