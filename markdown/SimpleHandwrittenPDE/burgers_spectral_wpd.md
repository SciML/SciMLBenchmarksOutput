...
The Burgers' partial differential equation is solved on the domain $[-L, L] \times [0, T] \in \mathbb R \times R,~L = 1,~T = 1$, with the following initial and boundary conditions:
```math
\begin{aligned}
    \partial_t u(t,x) & = -\partial_x u^2(t,x) + \nu \partial_x^2 u(t,x), \\
    u(0,x) & = \exp\left(\frac{x^2}{2 \times 0.01^2}\right), \\
    u(t,-L) & = u(t,L) = 0.
\end{aligned}
```
...
\begin{aligned}
    ...
    u(0,x_s) & = \exp\left(\frac{x_s^2}{2 \times 0.01^2}\right), \\
    u(t,-L) & = u(t,L) = 0.
\end{aligned}
```
...
```
5.415617 seconds (5.49 M allocations: 784.192 MiB, 8.26% gc time, 53.74% 
compilation time)
```
...
CNLF2
SBDF2
102.160120 seconds (37.54 M allocations: 11.363 GiB, 1.31% gc time, 8.92% c
ompilation time)
```


![](figures/burgers_spectral_wpd_4_1.png)
...
ETDRK2 (m=5)
ETDRK2 (m=20)
398.033109 seconds (74.60 M allocations: 80.187 GiB, 2.57% gc time, 2.09% c
ompilation time)
```


![](figures/burgers_spectral_wpd_5_1.png)
...
CNAB2 (Krylov linsolve)
ETDRK2 (m=5)
100.447396 seconds (16.33 M allocations: 11.598 GiB, 2.44% gc time, 1.93% c
ompilation time)
```
...
ARKODE4
ARKODE5
622.103451 seconds (330.49 M allocations: 55.659 GiB, 0.90% gc time, 1.36% 
compilation time)
```


![](figures/burgers_spectral_wpd_7_1.png)
...
ARKODE4
ARKODE5
622.005947 seconds (326.61 M allocations: 30.148 GiB, 0.49% gc time, 1.34% 
compilation time)
```
...
ETDRK4 (caching)
HochOst4 (caching)
152.877973 seconds (10.31 M allocations: 12.635 GiB, 0.55% gc time, 3.87% c
ompilation time)
```
...
ETDRK3 (caching)
ETDRK4 (caching)
214.947105 seconds (89.47 M allocations: 25.463 GiB, 1.17% gc time, 0.00% c
ompilation time)
```
...
SciMLBenchmarks.weave_file("benchmarks/SimpleHandwrittenPDE","burgers_spectral_wpd.jmd")
```
...
⌅ [864edb3b] DataStructures v0.18.22
  ...
  [6ad6398a] OrdinaryDiffEqBDF v1.7.0
  ...
  [e0540318] OrdinaryDiffEqExponentialRK v1.5.0
  [becaefa8] OrdinaryDiffEqExtrapolation v1.5.0
  [5960d6e9] OrdinaryDiffEqFIRK v1.13.0
  [101fe9f7] OrdinaryDiffEqFeagin v1.1.0
  [d3585ca7] OrdinaryDiffEqFunctionMap v1.2.0
  [d28bc4f8] OrdinaryDiffEqHighOrderRK v1.2.0
  [9f002381] OrdinaryDiffEqIMEXMultistep v1.4.0
  [521117fe] OrdinaryDiffEqLinear v1.3.0
  [1344f307] OrdinaryDiffEqLowOrderRK v1.3.0
  ...
  [af6ede74] OrdinaryDiffEqRKN v1.2.0
  [43230ef6] OrdinaryDiffEqRosenbrock v1.12.0
  [2d112036] OrdinaryDiffEqSDIRK v1.4.0
  [669c94d9] OrdinaryDiffEqSSPRK v1.3.0
  [e3e12d00] OrdinaryDiffEqStabilizedIRK v1.3.0
  [358294b1] OrdinaryDiffEqStabilizedRK v1.2.0
  [fa646aed] OrdinaryDiffEqSymplecticRK v1.4.0
  [b1df2697] OrdinaryDiffEqTsit5 v1.2.0
  [79d7bb75] OrdinaryDiffEqVerner v1.3.0
